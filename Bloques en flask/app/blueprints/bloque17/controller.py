...
def gestionar_ejercicio(num_ej):
    ...
    if request.method == 'POST':
        if num_ej == 1:
            ...
            nombre = request.form.get("nombre_input", "").strip()
            edad_str = request.form.get("edad_input", "").strip()
            notas_str = request.form.get("notas_input", "").strip()
            if campos_vacios(nombre, edad_str, notas_str):
                salida_consola = "⚠️ Debes completar nombre, edad y notas."
            else:
                notas = [float(x.strip()) for x in notas_str.split(",") if x.strip()]
                salida_consola = ejecutar_y_capturar(ejecutar_ejercicio2, nombre, int(edad_str), notas)
